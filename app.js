...
function getDefaultProperties() {
  // ✅ IDs ثابتة كـ strings لا تتعارض مع العقارات المضافة من المستخدمين
  return [
    { id:'default_1', title:'شقة فاخرة - حي النرجس', type:'شقة', location:'حي النرجس، القاهرة', price:5500, rooms:3, baths:2, area:120, icon:'🏢', phone:'0501234567', desc:'شقة حديثة وفاخرة بمواقع ممتازة', lat:30.0444, lng:31.2357, media:[], userId:'admin@ejaari.local', createdAt: Date.now()-600000 },
    { id:'default_2', title:'فيلا عصرية مع مسبح', type:'فيلا', location:'حي الملقا، الجيزة', price:18000, rooms:5, baths:4, area:400, icon:'🏡', phone:'0502345678', desc:'فيلا فخمة مع جميع التسهيلات', lat:30.0200, lng:31.2400, media:[], userId:'admin@ejaari.local', createdAt: Date.now()-500000 },
    { id:'default_3', title:'مكتب تجاري - القاهرة الجديدة', type:'مكتب', location:'القاهرة الجديدة', price:12000, rooms:0, baths:2, area:200, icon:'🏢', phone:'0503456789', desc:'مكتب تجاري بموقع استراتيجي', lat:30.0100, lng:31.2300, media:[], userId:'admin@ejaari.local', createdAt: Date.now()-400000 },
  ];
}
...
function saveProperties() {
  // ⚠️ هذه الدالة تكتب كل الـ properties المحلية دفعة واحدة
  // لا تستخدمها إلا لمزامنة offline → online
  if (db) {
    const updates = {};
...
// ✅ اكتب عقاراً واحداً فقط — لا تمس باقي العقارات على Firebase
function saveOneProperty(prop) {
  if (db) {
    db.ref('ejaari/properties/' + prop.id).set(prop)
      .catch(e => console.error('[FB] saveOneProperty error:', e));
  } else {
...
// ✅ احذف عقاراً واحداً فقط من Firebase مباشرة
function deleteOneProperty(id) {
  if (db) {
    db.ref('ejaari/properties/' + id).remove()
      .catch(e => console.error('[FB] deleteOneProperty error:', e));
    // الـ listener سيحدّث properties تلقائياً على كل الأجهزة
  } else {
...
function setupRealtimeListeners() {
  // ✅ listener على كل العقارات — يُشغَّل فوراً وعند أي تغيير من أي جهاز
  db.ref('ejaari/properties').on('value', snap => {
    if (snap.exists()) {
      const val = snap.val();
      // ✅ نقبل أي id (رقم أو string) ونزيل أي قيمة فارغة
      const all = Object.values(val).filter(p => p && p.id != null);
      // ✅ نزيل التكرار على أساس الـ id (حماية من race conditions)
      const seen = new Set();
...
async function initAppData() {
  ...
  try {
    ...
    if (firebaseOk) {
      try {
        ...
        users         = toArr(fbUsers);
        // ✅ تحميل العقارات من Firebase مع تنظيف أي قيم فارغة
        properties    = fbProps ? Object.values(fbProps).filter(p => p && p.id != null) : getDefaultProperties();
        messages      = toArr(fbMsgs);
        notifications = toArr(fbNotifs);

        // أول مرة فقط: اكتب العقارات الافتراضية إذا كانت قاعدة البيانات فارغة تماماً
        if (!fbProps) {
          const defProps = getDefaultProperties();
          // ✅ استخدم timestamp IDs للعقارات الافتراضية أيضاً لضمان التوافق
          const firstWrite = {};
          defProps.forEach((p, i) => {
            const stableId = 'default_' + (i + 1);
            const propWithId = { ...p, id: stableId };
            firstWrite['ejaari/properties/' + stableId] = propWithId;
          });
  ...
  updateUserUI();
  updateStats();
  // ✅ إذا Firebase متصل، الـ listener سيعمل renderAll() تلقائياً عند أول استقبال بيانات
  // لا نستدعي renderFeatured() هنا لأنها ستعرض بيانات قديمة قبل ما يجي الـ listener
  if (!db) renderFeatured();
...
function openChatByConvId(convId) {
  ...
  const propId = parts[0].replace('prop_', '');  // keep as string — IDs are now strings
  openChat(propId);
}
...
function sendMessage(convId, propId, owner, user) {
  ...
  if (isFirstMsg) {
    const propObj = properties.find(x => String(x.id) === String(propId));
...
function addProperty() {
  ...
  // ✅ ID فريد مضمون: timestamp + random → لا يمكن لجهازين يولّدا نفس الـ ID
  ...
  if (db) {
    // ✅ اكتب العقار على Firebase فقط — الـ listener سيحدّث كل الأجهزة تلقائياً
    db.ref('ejaari/properties/' + uniqueId).set(newProp)
      .then(() => console.log('[EJAARI] ✅ عقار أُضيف على Firebase:', uniqueId))
      .catch(e => {
        console.error('[EJAARI] ❌ Firebase add error:', e);
        showToast('❌ خطأ في حفظ العقار — تحقق من الاتصال');
      });
    // أضفه محلياً مؤقتاً للاستجابة الفورية (الـ listener سيصحح لو في فرق)
    properties.unshift(newProp);
    renderAll();
  } else {
    // وضع offline: localStorage
    properties.unshift(newProp);
    localStorage.setItem('ejaari_properties', JSON.stringify(properties));
    renderAll();
  }
...
function toggleStatus(propId, checkbox) {
  ...
  saveOneProperty(p);   // ✅ يكتب هذا العقار فقط — لا يمس الآخرين
  showToast(p.status === 'rented' ? '🔴 تم تحديث الحالة: تم التأجير' : '🟢 تم تحديث الحالة: متاح');
  if (!db) renderAll();  // مع Firebase الـ listener يحدّث تلقائياً
...
function removeProperty(id, event) {
  ...
  properties = properties.filter(p => p.id !== id);
  deleteOneProperty(id);  // ✅ يحذف من Firebase مباشرة — الـ listener يحدّث كل الأجهزة
  showToast('تم حذف العقار');
  if (!db) renderAll();   // مع Firebase الـ listener يتكفل بالتحديث
}
...
async function handleChangePw() {
  ...
  if (_changePwStep === 1) {
    ...
    if (sent) { showToast(`✅ تم إرسال رمز التحقق إلى ${sendTo}`); }
    // إذا فشل البريد، _showOtpDialog تعرض الرمز تلقائياً
...
//
// ✅ خطوات الإعداد (5 دقائق — مجاني — 200 بريد/شهر):
//
//  1️⃣  افتح https://www.emailjs.com وسجّل بأي إيميل
//  2️⃣  Email Services → Add Service → Gmail/Outlook → Create Service
//       احتفظ بـ Service ID  (مثال: service_abc123)
//  3️⃣  Email Templates → Create Template
//       Subject:  رمز التحقق — {{reset_code}}
//       Body:     مرحباً {{to_name}}، رمزك: {{reset_code}}
//       في Settings → To Email: {{to_email}}   ← مهم جداً
//       Save → احتفظ بـ Template ID (مثال: template_xyz789)
//  4️⃣  Account → General → Public Key → احتفظ به
...
function _ensureEmailJS() {
  ...
  if (EMAILJS_CONFIGURED) { emailjs.init({ publicKey: EMAILJS_PUBLIC_KEY }); _ejsReady = true; }
  return _ejsReady;
}

/**
 * إرسال رمز التحقق مباشرة لبريد المستخدم عبر EmailJS
 * إذا لم يكن EmailJS مضبوطاً → نافذة تعرض الرمز بوضوح
 */
async function sendEmailViaEmailJS(templateParams) {
  ...
  if (EMAILJS_CONFIGURED && _ensureEmailJS()) {
    try {
      ...
      if (resp.status === 200) {
        console.log('[Email] ✅ أُرسل إلى', to_email);
        return true;
      }
    } catch (err) {
  ...
  // Fallback: نافذة واضحة بالرمز — تعمل دائماً حتى بدون إعداد البريد
...
function _showOtpDialog(code, targetEmail) {
  ...
  box.id = '_otp_dialog';
  box.style.cssText = 'position:fixed;inset:0;z-index:999999;background:rgba(0,0,0,0.88);backdrop-filter:blur(10px);display:flex;align-items:center;justify-content:center;padding:20px';
  box.innerHTML = `
    <div style="background:#1e2535;border:1px solid rgba(201,168,76,0.5);border-radius:20px;padding:32px 28px;max-width:360px;width:100%;text-align:center;font-family:Tajawal,sans-serif">
      ...
      <div style="font-size:13px;color:#9aa3b8;margin-bottom:22px;line-height:1.7">
        استخدم هذا الرمز مباشرةً لإكمال العملية:
      </div>
      <div style="background:#0c0f14;border:2px solid #c9a84c;border-radius:16px;padding:20px 10px;font-size:38px;font-weight:900;color:#c9a84c;letter-spacing:12px;margin-bottom:16px;user-select:all;cursor:text">
        ${code}
      </div>
      <div style="font-size:12px;color:#6b7490;margin-bottom:24px">
        كان سيُرسل إلى: <span style="color:#c9a84c">${targetEmail}</span>
      </div>
      <button onclick="document.getElementById('_otp_dialog').remove()"
        style="background:#c9a84c;color:#0c0f14;border:none;border-radius:12px;padding:13px 36px;font-size:15px;font-weight:800;cursor:pointer;font-family:Tajawal,sans-serif;width:100%">
        ✅ حسناً — سأستخدم هذا الرمز
      </button>
    </div>`;
...
function renderMessagesPage() {
  ...
  list.innerHTML = convs.map(m => {
    ...
    let propTitle = '';
    if (m.propId) {
      const prop = properties.find(p => String(p.id) === String(m.propId));
