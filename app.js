...
function saveProperties() {
  if (db) {
    // كل عقار يُكتب كـ node منفصل — update لا set لضمان عدم مسح عقارات الأجهزة الأخرى
    const updates = {};
...
// حفظ عقار واحد فقط على Firebase بدون مساس بالعقارات الأخرى
function saveOneProperty(prop) {
  if (db) {
    db.ref('ejaari/properties/' + prop.id).update(prop)
      .catch(e => console.error('Firebase update one prop error:', e));
  } else {
...
// حذف عقار من Firebase مباشرة — لا يمس العقارات الأخرى
function deleteOneProperty(id) {
  if (db) {
    db.ref('ejaari/properties/' + id).remove()
      .catch(e => console.error('Firebase remove prop error:', e));
  } else {
...
function addProperty() {
  ...
  if (db) {
    // اكتب العقار الجديد على Firebase فقط — الـ realtime listener يحدّث properties تلقائياً على كل الأجهزة
    ...
    // أضفه محلياً مؤقتاً حتى يأتي الـ listener بالتحديث (لتجنب تأخير ظهوره)
...
function removeProperty(id, event) {
  ...
  properties = properties.filter(p => p.id !== id);
  deleteOneProperty(id);   // يحذف من Firebase مباشرة دون مساس بالعقارات الأخرى
...
async function handleChangePw() {
  ...
  if (_changePwStep === 1) {
    ...
    // إذا فشل الإرسال، sendEmailViaEmailJS تعرض الرمز تلقائياً في نافذة واضحة
...
// ==================== EMAIL CONFIG ====================
// ================================================================
// إعداد EmailJS لإرسال رمز التحقق مباشرة إلى بريد المستخدم
//
// خطوات الإعداد (مجاني — 200 بريد/شهر):
// 1. سجّل على https://www.emailjs.com
// 2. أنشئ Email Service (Gmail / Outlook) → احفظ الـ Service ID
// 3. أنشئ Email Template بهذه المتغيرات:
//      {{to_name}}   {{to_email}}   {{reset_code}}   {{platform_name}}
//    وفي حقل "To Email" اكتب: {{to_email}}
// 4. انسخ الـ Public Key من Account → API Keys
// 5. استبدل القيم الثلاثة أدناه
// ================================================================
...
// تهيئة EmailJS عند أول استخدام
let _emailjsInited = false;
function _initEmailJS() {
  if (_emailjsInited || !EMAILJS_IS_CONFIGURED) return;
...
/**
 * إرسال رمز التحقق مباشرة إلى بريد المستخدم عبر EmailJS.
 * الـ to_email يُحدَّد من بريد المستخدم المُدخَل — ليس بريد الأدمين.
 * إذا لم يكن EmailJS مضبوطاً، يعرض الرمز على الشاشة كـ fallback.
 */
async function sendEmailViaEmailJS(templateParams) {
  ...
  if (!EMAILJS_IS_CONFIGURED) {
    // Fallback مرئي: أظهر الرمز في نافذة واضحة بدلاً من toast عابر
    console.warn('[Email] EmailJS غير مضبوط — fallback مرئي');
    _showOtpFallback(reset_code, to_email);
    return false;
  }
  ...
  if (typeof emailjs === 'undefined') {
    console.error('[Email] EmailJS SDK غير محمّل');
    _showOtpFallback(reset_code, to_email);
    return false;
  }
  ...
  try {
    const result = await emailjs.send(
      ...
      {
        to_name:       to_name,
        to_email:      to_email,        // ← بريد المستخدم الفعلي
        reset_code:    reset_code,
        platform_name: platform_name || 'إيجاري | Ejaari'
      }
    );
    if (result.status === 200) {
      console.log('[Email] ✅ أُرسل بنجاح إلى', to_email);
      return true;
    } else {
    ...
    console.error('[Email] EmailJS exception:', err);
    _showOtpFallback(reset_code, to_email);
    return false;
  }
}
...
/**
 * عرض رمز التحقق في نافذة واضحة على الشاشة كـ fallback
 * عندما يكون EmailJS غير مضبوط أو فشل الإرسال
 */
function _showOtpFallback(code, email) {
  // أزل أي نافذة fallback سابقة
  const old = document.getElementById('otp-fallback-overlay');
  if (old) old.remove();

  const overlay = document.createElement('div');
  overlay.id = 'otp-fallback-overlay';
  ...
  overlay.innerHTML = `
    <div style="background:#1e2535;border:1px solid rgba(201,168,76,0.4);border-radius:20px;
                padding:32px;max-width:380px;width:100%;text-align:center;font-family:Tajawal,sans-serif">
      <div style="font-size:40px;margin-bottom:12px">🔐</div>
      <div style="font-size:18px;font-weight:800;color:#e8eaf0;margin-bottom:8px">رمز التحقق الخاص بك</div>
      <div style="font-size:13px;color:#9aa3b8;margin-bottom:20px;line-height:1.6">
        لم يتم إرسال الرمز إلى البريد الإلكتروني بسبب عدم إعداد خدمة البريد.<br>
        استخدم هذا الرمز مباشرةً:
      </div>
      <div style="background:#0c0f14;border:2px solid #c9a84c;border-radius:14px;
                  padding:18px 24px;font-size:36px;font-weight:900;color:#c9a84c;
                  letter-spacing:10px;margin-bottom:20px;user-select:all">
        ${code}
      </div>
      <div style="font-size:12px;color:#6b7490;margin-bottom:20px">
        ✉️ كان سيُرسل إلى: <span style="color:#c9a84c">${email}</span>
      </div>
      <button onclick="document.getElementById('otp-fallback-overlay').remove()"
        style="background:#c9a84c;color:#0c0f14;border:none;border-radius:12px;
               padding:12px 32px;font-size:15px;font-weight:700;cursor:pointer;font-family:Tajawal,sans-serif">
