...
// Cloud Sync Edition: All data stored in window.storage (cross-device)
// ========================================
...
// window.storage is the persistent cross-device store.
// Falls back to localStorage if not available (local dev).
...
// ==================== INIT: Load all data from cloud ====================
async function initAppData() {
  showLoadingOverlay(true);
  try {
    // Load session (user stays logged in per-device via localStorage only)
    try { currentUser = JSON.parse(localStorage.getItem('ejaari_user')) || null; } catch(e) {}

    // Load shared cloud data
    const [cloudUsers, cloudProps, cloudMsgs, cloudNotifs] = await Promise.all([
      cloudGet('ejaari_users'),
      cloudGet('ejaari_properties'),
      cloudGet('ejaari_messages'),
      cloudGet('ejaari_notifications'),
    ]);

    users         = cloudUsers   || [];
    properties    = cloudProps   || getDefaultProperties();
    messages      = cloudMsgs    || [];
    notifications = cloudNotifs  || [];

    // Seed default properties to cloud if first time
    if (!cloudProps) {
      await cloudSet('ejaari_properties', properties);
    }
  } catch(e) {
    console.error('Cloud load error:', e);
    // Graceful fallback to localStorage
    try { users         = JSON.parse(localStorage.getItem('ejaari_users')) || []; } catch(e2) {}
    try { properties    = JSON.parse(localStorage.getItem('ejaari_properties')) || getDefaultProperties(); } catch(e2) {}
    try { messages      = JSON.parse(localStorage.getItem('ejaari_messages')) || []; } catch(e2) {}
    try { notifications = JSON.parse(localStorage.getItem('ejaari_notifications')) || []; } catch(e2) {}
  }
  _appReady = true;
...
function showLoadingOverlay(show) {
  ...
  if (!overlay) {
    ...
    overlay.style.cssText = 'position:fixed;inset:0;background:var(--bg);z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;font-family:Tajawal,sans-serif;';
    overlay.innerHTML = `<div style="font-size:48px">🏠</div><div style="color:var(--gold);font-size:20px;font-weight:700">إيجاري</div><div style="color:var(--text2);font-size:14px">جارٍ تحميل البيانات...</div><div style="width:48px;height:4px;background:var(--border2);border-radius:4px;overflow:hidden;margin-top:8px"><div id="app-load-bar" style="height:100%;background:var(--gold);animation:loadBar 1.5s ease-in-out infinite;border-radius:4px"></div></div><style>@keyframes loadBar{0%{width:0%}50%{width:80%}100%{width:100%}}</style>`;
...
// ==================== STORAGE HELPERS (Cloud) ====================
function saveProperties() {
  cloudSet('ejaari_properties', properties).catch(console.error);
  updateStats();
}
function saveMessages() {
  cloudSet('ejaari_messages', messages).catch(console.error);
}
function saveNotifications() {
  cloudSet('ejaari_notifications', notifications).catch(console.error);
}
function saveUsers() {
  cloudSet('ejaari_users', users).catch(console.error);
}
...
function navigate(page) {
  ...
  window.scrollTo(0, 0);
  closeHamburger();
  // Refresh cloud data in background on every navigation
  refreshFromCloud();
}

async function refreshFromCloud() {
  try {
    const [cloudProps, cloudMsgs, cloudNotifs, cloudUsers] = await Promise.all([
      cloudGet('ejaari_properties'),
      cloudGet('ejaari_messages'),
      cloudGet('ejaari_notifications'),
      cloudGet('ejaari_users'),
    ]);
    if (cloudProps)   { properties    = cloudProps;   renderAll(); updateStats(); }
    if (cloudMsgs)    { messages      = cloudMsgs; }
    if (cloudNotifs)  { notifications = cloudNotifs; updateNotifBadge(); }
    if (cloudUsers)   { users         = cloudUsers; }
  } catch(e) {}
}
...
async function handleChangePw() {
  ...
  if (currentUser.email === 'admin@ejaari.local') {
    ...
    if (current !== stored) {
      if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = 'حفظ كلمة المرور الجديدة'; }
      showToast('❌ كلمة المرور الحالية غير صحيحة'); return;
    }
    if (adminRecord) adminRecord.password = newPw;
    else users.push({ email: 'admin@ejaari.local', name: 'المدير', password: newPw, type: 'admin' });
    saveUsers();
    // إرسال إيميل تأكيد حقيقي للأدمين
    await sendEmailViaEmailJS({
      to_email:      ADMIN_REAL_EMAIL,
      to_name:       'مدير المنصة',
      reset_code:    '(تم التغيير من داخل الحساب)',
      platform_name: 'إيجاري | Ejaari',
      account_email: 'admin@ejaari.local'
    });
    ...
    if (!record) {
      if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = 'حفظ كلمة المرور الجديدة'; }
      showToast('❌ لم يتم العثور على الحساب'); return;
    }
    ...
    await sendEmailViaEmailJS({
      ...
      to_name:       currentUser.name || currentUser.email.split('@')[0],
      reset_code:    '(تم التغيير من داخل الحساب)',
      platform_name: 'إيجاري | Ejaari',
...
// ==================== EMAILJS CONFIG ====================
// EmailJS - Public Key & Service/Template IDs
const EMAILJS_PUBLIC_KEY  = 'YOUR_EMAILJS_PUBLIC_KEY';   // ← استبدل بـ Public Key من EmailJS
const EMAILJS_SERVICE_ID  = 'YOUR_SERVICE_ID';           // ← Service ID من EmailJS
const EMAILJS_TEMPLATE_ID = 'YOUR_TEMPLATE_ID';          // ← Template ID من EmailJS
const ADMIN_REAL_EMAIL    = 'johnjohan10499@gmail.com';  // البريد الحقيقي للأدمين

/**
 * إرسال إيميل حقيقي عبر EmailJS
 * templateParams يجب أن تحتوي: to_email, to_name, reset_code, platform_name
 */
async function sendEmailViaEmailJS(templateParams) {
  // EmailJS SDK يُحمَّل في الصفحة عبر CDN
  if (typeof emailjs === 'undefined') {
    console.error('EmailJS SDK غير محمَّل');
    return false;
  }
  try {
...
async function sendResetCode() {
  ...
  const sendTo   = isAdmin ? ADMIN_REAL_EMAIL : email;
  ...
  if (!sent) {
    // Fallback: أظهر الكود في الـ toast لو EmailJS غير مضبوط بعد
    showToast(`⚠️ تعذّر إرسال الإيميل — رمزك: ${_resetCode}`);
    console.warn(`[FALLBACK] Reset code for ${email}: ${_resetCode} → would send to ${sendTo}`);
  } else {
