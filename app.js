...
function saveProperties() {
  if (db) {
    // نحول الـ array لـ object بـ id كـ key لضمان عدم ضياع البيانات
    const obj = {};
    properties.forEach(p => { obj['p_' + p.id] = p; });
    fbSet('ejaari/properties', obj);
  } else {
...
function setupRealtimeListeners() {
  // العقارات: أي إضافة أو تعديل يظهر فوراً على كل الأجهزة
  fbListen('ejaari/properties', data => {
    if (data) {
      const raw = Array.isArray(data) ? data : Object.values(data);
      // تنقية: أزل أي null/undefined وتأكد من وجود id
      properties = raw.filter(p => p && p.id);
    } else {
      properties = [];
    }
    renderAll();
  ...
  try {
    ...
    if (firebaseOk) {
      try {
        ...
        const toArr = (d) => d ? (Array.isArray(d) ? d : Object.values(d)).filter(Boolean) : [];
        users         = toArr(fbUsers);
        properties    = fbProps ? toArr(fbProps).filter(p => p && p.id) : getDefaultProperties();
...
function openChangePw() {
  ...
  const sendTo = currentUser.email === 'admin@ejaari.local' ? ADMIN_REAL_EMAIL : currentUser.email;
  if (sub) sub.textContent = `سيتم إرسال رمز التحقق إلى: ${sendTo}`;
...
async function handleChangePw() {
  ...
  if (_changePwStep === 1) {
    ...
    if (!sent) {
      // Fallback إذا EmailJS غير مضبوط
      console.warn('[EJAARI] Change-PW OTP:', _changePwOtp, '→', sendTo);
      showToast(`⚠️ تعذر إرسال الإيميل — رمز التحقق: ${_changePwOtp}`);
    } else {
...
async function sendEmailViaEmailJS(templateParams) {
  ...
  if (
    ...
    console.warn('[EmailJS] لم يتم ضبط الإعدادات بعد');
    return false;
  }

  // أولاً: حاول عبر SDK لو محمّل
  if (typeof emailjs !== 'undefined') {
    try {
      emailjs.init(EMAILJS_PUBLIC_KEY);
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams);
      console.log('[EmailJS] ✅ أُرسل عبر SDK');
      return true;
    } catch (err) {
      console.warn('[EmailJS SDK] فشل، نجرب REST API:', err);
    }
  }

  // ثانياً: REST API مباشرة (لا يحتاج SDK)
  try {
    ...
    if (res.ok) {
      console.log('[EmailJS] ✅ أُرسل عبر REST API');
      return true;
    } else {
      const txt = await res.text();
      console.error('[EmailJS REST]', res.status, txt);
      return false;
    }
  } catch (err) {
