...
function addProperty() {
  ...
  const amenities = [];
  document.querySelectorAll('#amenities-group .check-item.checked input').forEach(cb => {
  ...
  const utilities = [];
  document.querySelectorAll('#utilities-group .check-item.checked input').forEach(cb => {
    utilities.push(cb.value);
  });
  ...
  document.querySelectorAll('#amenities-group input, #utilities-group input').forEach(cb => cb.checked = false);
...
function toggleCheck(labelEl) {
  // prevent double-fire from inner elements
  labelEl.classList.toggle('checked');
  const cb = labelEl.querySelector('input[type="checkbox"]');
  if (cb) cb.checked = labelEl.classList.contains('checked');
}
