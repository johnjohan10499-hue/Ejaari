...
// ==================== EMAIL CONFIG ====================
// البريد الحقيقي للأدمين — يستقبل كل إشعارات تغيير كلمة المرور
const ADMIN_REAL_EMAIL = 'johnjohan10499@gmail.com';

// Web3Forms Access Key — مجاني بدون إعداد معقد
// احصل على مفتاحك من: https://web3forms.com (أدخل إيميلك واضغط "Get your Access Key")
const WEB3FORMS_KEY = 'e3202659-f983-49b3-b574-1d4447f752e4'; // ← استبدل بمفتاحك

/**
 * إرسال إيميل حقيقي عبر Web3Forms
 * يرسل من خوادم Web3Forms باسم "إيجاري" إلى to_email
 */
async function sendEmailViaEmailJS(templateParams) {
  ...
  if (!WEB3FORMS_KEY || WEB3FORMS_KEY === 'YOUR_WEB3FORMS_ACCESS_KEY') {
    console.warn('[Email] WEB3FORMS_KEY غير مضبوط — fallback للـ toast');
    return false;
  }

  const subject = `رمز التحقق من ${platform_name || 'إيجاري'} — ${reset_code}`;
  const body = `
مرحباً ${to_name}،

تلقينا طلباً لتغيير كلمة المرور لحسابك على منصة إيجاري.

🔐 رمز التحقق: ${reset_code}

الرمز صالح لمدة 10 دقائق.
إذا لم تطلب تغيير كلمة المرور، تجاهل هذا البريد.
...
— فريق إيجاري
  ...
  try {
    ...
    formData.append('to',         to_email);           // المستلم
    formData.append('from_name',  'إيجاري | Ejaari');
    formData.append('replyto',    ADMIN_REAL_EMAIL);   // johnjohan10499@gmail.com
    ...
    if (data.success) {
      ...
      console.error('[Email] Web3Forms error:', data.message);
      return false;
    }
  } catch (err) {
