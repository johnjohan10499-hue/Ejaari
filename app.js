...
function saveProperties() {
  fbSet('ejaari/properties', properties);
  updateStats();
}
function saveMessages() {
  fbSet('ejaari/messages', messages);
}
function saveNotifications() {
  fbSet('ejaari/notifications', notifications);
}
function saveUsers() {
  fbSet('ejaari/users', users);
}

// ==================== REALTIME LISTENERS ====================
// يستمع للتغييرات من الأجهزة الأخرى ويحدّث الواجهة تلقائياً
function setupRealtimeListeners() {
  fbListen('ejaari/properties', data => {
    if (data) {
      properties = Array.isArray(data) ? data : Object.values(data);
      renderAll();
      updateStats();
    }
  });
  fbListen('ejaari/messages', data => {
    if (data) messages = Array.isArray(data) ? data : Object.values(data);
  });
  ...
  fbListen('ejaari/users', data => {
    if (data) users = Array.isArray(data) ? data : Object.values(data);
  });
}
...
function showLoadingOverlay(show) {
  ...
  if (!el) {
    ...
    el.innerHTML = `
      <style>
        #app-loading-overlay {
          position:fixed;inset:0;background:#0c0f14;z-index:99999;
          display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;
          font-family:'Tajawal',sans-serif;
        }
        #app-loading-overlay .spin {
...
async function initAppData() {
  ...
  // تحميل جلسة المستخدم (تبقى محلية لكل جهاز - هذا صحيح)
  ...
  if (firebaseOk) {
    ...
    const [fbUsers, fbProps, fbMsgs, fbNotifs] = await Promise.all([
      fbGet('ejaari/users'),
      fbGet('ejaari/properties'),
      fbGet('ejaari/messages'),
      fbGet('ejaari/notifications'),
    ]);

    users         = fbUsers   ? (Array.isArray(fbUsers)   ? fbUsers   : Object.values(fbUsers))   : [];
    properties    = fbProps   ? (Array.isArray(fbProps)   ? fbProps   : Object.values(fbProps))   : getDefaultProperties();
    messages      = fbMsgs    ? (Array.isArray(fbMsgs)    ? fbMsgs    : Object.values(fbMsgs))    : [];
    notifications = fbNotifs  ? (Array.isArray(fbNotifs)  ? fbNotifs  : Object.values(fbNotifs))  : [];

    // إذا أول مرة - احفظ العقارات الافتراضية على Firebase
    if (!fbProps) {
      await fbSet('ejaari/properties', properties);
    }

    // شغّل الاستماع الفوري للتغييرات
    setupRealtimeListeners();
  } else {
    // fallback للـ localStorage لو Firebase مش مضبوط
